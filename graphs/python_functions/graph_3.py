...
def build_graph_with_entities(df, topic_names):
    ...
    - Node weight: Total occurrences of an entity across all articles.
    - Edge weight: Total occurrences of an entity pair across all articles.
    - MultiGraph allows multiple edges for different topics.
    ...
    Returns:
        nx.MultiGraph: Graph of entities with multiple edges per topic and weighted nodes.
    """
    G = nx.MultiGraph() 

    word_color_map = {}
    colors = ['#1f78b4', '#33a02c', '#e31a1c', '#ff7f00', '#6a3d9a']
    column_types = ["loc-l", "org-l", "per-l"]

    for col_idx, col in enumerate(column_types):
        words = df[col].explode().dropna().unique()
        for word in words:
            word_color_map[word] = colors[col_idx % len(colors)]

    entity_counts = Counter()

    global_entity_pair_counts = Counter()
    topic_entity_pair_counts = {} 
    ...
    for _, row in df.iterrows():
        ...
        for entity1, entity2 in entity_pairs:
            global_entity_pair_counts[(entity1, entity2)] += 1

            if (entity1, entity2, topic_name) not in topic_entity_pair_counts:
                topic_entity_pair_counts[(entity1, entity2, topic_name)] = 1
            else:
                topic_entity_pair_counts[(entity1, entity2, topic_name)] += 1

    # Add nodes with weights (total occurrences)
    for entity, count in entity_counts.items():
        G.add_node(entity, weight=count, color=word_color_map.get(entity, '#000000'))

    # Add edges with weights based on global occurrences
    for (entity1, entity2), total_weight in global_entity_pair_counts.items():
        for topic_name in topic_names.values():
            if (entity1, entity2, topic_name) in topic_entity_pair_counts:
                topic_weight = topic_entity_pair_counts[(entity1, entity2, topic_name)]
                G.add_edge(entity1, entity2, weight=total_weight, topic=topic_name, topic_weight=topic_weight)
